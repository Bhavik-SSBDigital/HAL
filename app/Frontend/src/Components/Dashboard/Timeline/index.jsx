...
const Timeline = () => {
  ...
  const handleGoBack = () => {
    navigate('/dashboard/perticularBranch')
  }
  ...
  const selectedDepData = data?.workflows?.find(item => item.departmentName === selectedDep);
  const skipped = [1, 2];
  return (
    <div
    ...
    >
      ...
      {Object.keys(data).length ?
                        ...
                        {data?.workflows
                          ...
                          ?.workFlow?.map((label, index) => (
                            <Step
                              ...
                              completed={selectedDepData?.skippedStepNumbers?.includes(index + 1) ? false : selectedDepData?.lastStepDone >= index + 1 ? true : false}
                            >
                              <StepLabel>
                                <p>{label.work} ({label?.users?.map((user) => user.user).join(', ')})</p>
                                <p style={{ mt: "10px" }}>{(selectedDepData?.skippedStepNumbers?.includes(index + 1) ? <p style={{ color: "red" }}>Skipped</p> : selectedDepData?.lastStepDone >= index + 1 ? "" : <p style={{ color: "orange" }}>Pending</p>)}</p>
                                <p>{(selectedDepData?.isCompleted && selectedDepData?.lastStepDone === index + 1) ? "Process Completed Here" : ""}</p>
                              </StepLabel>
                            </Step>
                          ))}
                      </Stepper>
        ...
        </Stack> : <Stack flexDirection="row" justifyContent="center"> <Typography sx={{ p: 2, my: 'auto' }} textAlign="center">Error Getting History</Typography> <Button onClick={handleGoBack}>Go Back</Button></Stack>}
