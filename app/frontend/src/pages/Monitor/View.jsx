...
export default function View() {
  ...
  const sendToClerk = async (workFlowToBeFollowed) => {
    ...
    try {
      const res = await axios.post(
        clerkUrl,
        { work: selectedWork, workFlowToBeFollowed: workFlowToBeFollowed, remarks },
        {
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("accessToken")}`,
          },
  ...
  const rejectProcess = async (processId) => {
    ...
    const rejectProcessUrl =
      backendUrl + `/rejectFromHeadOffice/${processId}`;
    try {
      const res = await axios.post(
        rejectProcessUrl,
        {
      ...
      {loading ? <ComponentLoader /> : <Stack flexDirection="row">
            ...
            <Box sx={{ backgroundColor: "white", border: "1px solid lightgray" }}>
              <Stack>
                ...
                >
                  ...
                  />
                  <InfoRow label="Document Path" value={data?.documentsPath} />
                  <InfoRow
                    label="Created Date"
                    value={moment(data?.createdAt).format("DD-MM-YYYY hh:mm A")}
                  />
                  <InfoRow
                    label="Previous Step"
                    value={
        ...
        <Dialog open={sendToClerkMenu}>
          ...
          <Box width={300} padding={1}>
            ...
            <Typography variant="body1" mt={1}>Remarks :</Typography>
            <TextField fullWidth multiline rows={2} onChange={(e) => setRemarks(e.target.value)} />
          </Box>
          <DialogActions
            sx={{
              display: "flex",
