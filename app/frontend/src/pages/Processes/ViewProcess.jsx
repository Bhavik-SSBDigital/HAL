...
export default function ViewProcess(props) {
        ...
        try {
            ...
            if (res.status === 200) {
                ...
                setProcessData((prevProcessData) => {
                    ...
                    const updatedProcessData = { ...prevProcessData, hasUserDoneAnythingAfterReceivingProcess: true, isForwardable: res?.data?.isForwardable };
    ...
    const handleRejectFile = async (processId, fileId) => {
        ...
        try {
            ...
            if (res.status === 200) {
                ...
                setProcessData((prevProcessData) => {
                    const updatedProcessData = { ...prevProcessData, hasUserDoneAnythingAfterReceivingProcess: true, isRevertable: res?.data?.isRevertable };
    ...
    const disableNext = () => {
        ...
        if (processData?.workFlow[processData?.currentStepNumber - 1]?.work !== 'e-sign') {
            return false;
        }
        ...
        processData?.documents?.some(document => {
            ...
            const isRejectedByCurrentUser = rejection &&
            ...
            if (rejection && processData?.rejectedDocIdsInPrevLogs?.includes(details._id)) {
                return false;
            }
        ...
        <>
            {loading ? (
                ...
                <Stack flexDirection="row">
                                ...
                                <Stack sx={{ padding: '5px' }}>
                                    <Box>
                                        <Stack>
                                            ...
                                            >
                                                <InfoRow label="Process Name" value={processData?.name} />
                                                <InfoRow
                                                    label="File Name"
                                                    value={
                                                        processData?.documents?.length <= 4
                                                            ? processData?.documents?.map((item, index) => {
                                                                ...
                                                                const work = name.split('_')[1]; // Assuming workName can be extracted
                                                                return (
                                                            ...
                                                            : <Button onClick={handleOpenFilesList} size='small'>View</Button>
                                                    }
                                                />                                                <InfoRow
                                                    label="Status"
                                                    value={
                                            ...
                                            <Box sx={{ padding: '5px' }}>
                                                ...
                                                >
                                                    ...
                                                    >
                                                        ...
                                                        <Box>
                                                            <input
                                                                style={{ border: '1px solid lightgray', width: "100%", padding: "10px" }}
                                                                type="file"
                                ...
                                {processData.currentActorUser === username ||
                                    ...
                                    <Stack alignItems="center">
                                        <Button
                                            ...
                                            onClick={pickProcess}
                                        >
                                            {pickProcessLoading ? <CircularProgress size={24} /> : "Pick This Process"}
                                        </Button>
                                    </Stack>
                                )}
                            </div>
                    ...
                    <Dialog maxWidth="md" fullWidth open={openFilesList} onClose={handleCloseFilesList}>
                        <DialogTitle>Files</DialogTitle>
                        <IconButton sx={{ position: "absolute", right: "5px", top: "5px" }} onClick={handleCloseFilesList}>
                            <IconSquareRoundedX />
                        </IconButton>
                        <DialogContent>
                            <TableContainer sx={{ maxHeight: "400px", overflow: 'auto', border: '1px solid lightgray' }}>
                                <Table>
                                    <TableHead>
                                        <TableRow>
                                    ...
                                    <TableBody>
                                        {processData?.documents?.length ? processData?.documents?.map((item, index) => {
                                            const name = item.details.name;
                                            return (
                                                <TableRow key={index}>
