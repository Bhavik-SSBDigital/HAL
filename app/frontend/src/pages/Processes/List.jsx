...
export default function List() {
    const { setWork, pickedProcesses, setNotifications, notifications } = sessionData();
    ...
    const { data, error, isLoading, isFetching } =
        useQuery('pendingProcesses', fetchProcesses, {
            cacheTime: 12000,
            staleTime: 12000,
    ...
    const filteredData = data?.processes
        ?.filter((item) =>
                                        ...
                                        ?.map((row, index) => (
                                            <>
                                                <TableRow key={index} className={styles.tableRow}>
                                                    ...
                                                    <TableCell className={styles.cell}>
                                                        {getWorkName(row.files).map((fileName, index1) => <Typography key={index1}>- {fileName}</Typography>)}
                                                        {row?.files?.length > 4 && (
                                                            <Button onClick={() => handleViewFileList(row.files)} size='small'>View More</Button>
                                                        )}
                                                    </TableCell>
                                                ...
                                                {/* <TableRow>
                                                    <TableCell colSpan={4}>
                                                        ...
                                                        {row?.files?.map((file, index) => <Typography key={index}>{index + 1}. {file}</Typography>)}</TableCell>
                                                </TableRow> */}
                                            </>
